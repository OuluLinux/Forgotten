...
template <class T> using StdShared = std::shared_ptr<T>;
...
int MyASTConsumer::InitializeCI(CompilerInstance& ci,
	...
	StdShared<clang::CompilerInvocation> Invocation = std::make_shared<clang::CompilerInvocation>();
	...
	clang::InputKind kind(clang::Language::CXX);
	llvm::Triple lang_std(llvm::sys::getDefaultTargetTriple());
	clang::PreprocessorOptions pp_opt;
	
	ci.getInvocation().setLangDefaults(ci.getLangOpts(), kind, lang_std, pp_opt);
...
void MyASTConsumer::DumpContent(std::string const& file_name) {
	...
	DEBUG_ASTCONSUMER(dbgs() << "Current file name in AST comsumer is: "
					  << current_file;);
	llvm::ErrorOr<const FileEntry*> pFile = ci.getFileManager().getFile(file_name.c_str());
