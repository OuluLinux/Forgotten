...
# Script that can be run to set up a Forgotten OS repository for building with minimal
# effort.

set -e

echo "Forgotten OS build script"
echo "This script will ask a couple questions and then automatically install"
echo "dependencies and compile Forgotten for you."
echo
...
$script_dir/scripts/checkBuildSystemNoInteractive.pl amd64-fos $script_dir/fos-compiler $compiler_build_options
...
if [ $? != 0 ]; then
    ...
    if [[ $(git diff --shortstat 2> /dev/null | tail -n1) == "" ]]; then
        echo
        echo "Please wait, checking for a newer revision of the Forgotten source code."
...
echo
echo "Configuring the Forgotten Updater..."

$script_dir/setup_fu.py
$script_dir/run_fu.py sync

echo
echo "Installing a base set of packages..."

$script_dir/run_fu.py install fos-base
$script_dir/run_fu.py install libpng
$script_dir/run_fu.py install libfreetype
$script_dir/run_fu.py install libiconv
$script_dir/run_fu.py install zlib

$script_dir/run_fu.py install bash
$script_dir/run_fu.py install coreutils

set -e

echo
echo "Beginning the Forgotten build."
echo

# Build Forgotten.
scons CROSS=$script_dir/compilers/dir/bin/amd64-fos-

cd "$old"

echo
echo
echo "Forgotten is now ready to be built without running this script."
echo "To build in future, run the following command in the '$script_dir' directory:"
echo "scons CROSS=$script_dir/fos-compiler/bin/amd64-fos-"
echo
echo "If you wish, you can continue to run this script. It won't ask questions"
echo "anymore, unless you remove the '.build_os' file in '$script_dir'."
echo
echo "You can also run scons --help for more information about options."
echo
echo "Patches should be posted in the issue tracker at https://github.com/UltimateScript/Forgotten/issues"
echo "Support can be found in #fos on irc.quakenet.org."
echo
echo "Keep it going!"
