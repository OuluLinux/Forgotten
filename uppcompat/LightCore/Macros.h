...
LOG("}"); \
	...
	extern "C" int main(int argc, const char** argv) {\
		::UScript::SetExeFilePath(argv[0]); \
		::UScript::SeedRandom(); \
		::UScript::ParseCommandLine(argc, argv); \
		::UScript::ReadCoreCmdlineArgs(); \
		::UScript::RunInitBlocks(); \
		AppMain(); \
		::UScript::Thread::ShutdownThreads(); \
		::UScript::RunExitBlocks(); \
		return ::UScript::GetExitCode(); \
	} \
	\
	void AppMain()
#endif
