...
extern void fos_module_load(char *file);
...
int load_module(char *name)
{
    ...
    if(fos_module_is_loaded(modname))
    {
    ...
    for(size_t i = 0; deps[i]; i++)
    {
        ...
        if(fos_module_is_loaded(depname))
            continue;
        if(!load_module(depname) && fos_module_is_loaded(depname))
            continue;
        printf("Dependency %s of %s couldn't been loaded!\n", depname, modname);
        return -1;
    }
    // Finally load the module
    fos_module_load(file);
    if(fos_module_is_loaded(modname))
        return 0;
