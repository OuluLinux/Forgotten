...
namespace Forgotten
{
	...
	class NetworkDevice
	{
        public:
		    ...
		    string name;
		    IpAddress ipv4; /// \todo StationInfo in Forgotten
		    IpAddress subnet;
		    IpAddress gateway;
...
int main(int argc, char* argv[])
{
	...
	vector<Forgotten::NetworkDevice> devices = Forgotten::Network::instance().getDevices();
	for(vector<Forgotten::NetworkDevice>::iterator it = devices.begin(); it != devices.end(); ++it)
	{
		Forgotten::NetworkDevice dev = *it;
		cout << "Device '" << dev.name << "':" << endl;
		cout << "\tIPv4: " << dev.ipv4.toString() << ", Subnet Mask: " << dev.subnet.toString() << ", Gateway: " << dev.gateway.toString() << endl;
		cout << "\tDNS servers:";
		for(vector<Forgotten::IpAddress>::iterator dns = dev.dns.begin(); dns != dev.dns.end(); ++dns)
			cout << " " << (*dns).toString();
		cout << endl;
	}
